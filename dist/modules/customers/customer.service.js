...
class CustomerService {
    ...
    async create(sellerId, payload) {
        ...
        if (!sellerId) {
            throw new errorHandler_1.ApiError('Seller id required', 400);
        }
        ...
        const existingCustomer = await db('customers')
            ...
            .first();
        if (existingCustomer) {
            throw new errorHandler_1.ApiError('Customer already exists for this seller', 409);
        }
        const existingUser = await db('users').where({ mobile: data.mobile }).first();
        if (existingUser && existingUser.role !== 'customer') {
            throw new errorHandler_1.ApiError('Mobile already used by another account', 400);
        }
        const userId = existingUser ? existingUser.id : (0, crypto_1.randomUUID)();
        let temporaryPin = null;
        if (!existingUser) {
            temporaryPin = '123456';
            const pinHash = await bcryptjs_1.default.hash(temporaryPin, 10);
        ...
        if (data.products && data.products.length > 0) {
            for (const productSelection of data.products) {
                ...
                const product = await db('products')
                    ...
                    .first();
                if (!product) {
                    throw new errorHandler_1.ApiError(`Product ${productSelection.productId} not found for this seller`, 404);
                }
    ...
    async update(customerId, payload) {
        ...
        await db('customers')
            ...
            daily_quantity: typeof data.dailyQuantity !== 'undefined' ? data.dailyQuantity : existing.daily_quantity,
        ...
        if (data.products && Array.isArray(data.products)) {
            ...
            if (data.products.length > 0) {
                ...
                if (validProducts.length !== productIds.length) {
                    throw new errorHandler_1.ApiError('One or more products do not belong to this seller', 400);
                }
            }
    ...
    async setPauseWindow(customerId, payload) {
    ...
    async getPauseHistory(customerId) {
        const db = (0, connection_1.getDb)();
        const customer = await db('customers').where({ id: customerId }).first();
        if (!customer) {
            throw new errorHandler_1.ApiError('Customer not found', 404);
        ...
        if (customer.pause_from && customer.pause_to) {
            const currentInHistory = history.some((h) => h.pauseFrom === customer.pause_from && h.pauseTo === customer.pause_to);
            if (!currentInHistory) {
                const today = new Date().toISOString().split('T')[0];
    ...
    async cancelPause(customerId, pauseId) {
        const db = (0, connection_1.getDb)();
        const customer = await db('customers').where({ id: customerId }).first();
        if (!customer) {
            throw new errorHandler_1.ApiError('Customer not found', 404);
        ...
        if (!historyEntry) {
            throw new errorHandler_1.ApiError('Pause entry not found', 404);
        }
    ...
    async toggleStatus(customerId, payload) {
    ...
    async resetPin(customerId) {
        ...
        if (!customer.user_id) {
            throw new errorHandler_1.ApiError('Customer login not configured', 400);
        }
        const fixedPin = '123456';
        const pinHash = await bcryptjs_1.default.hash(fixedPin, 10);
        await db('users')
            .where({ id: customer.user_id })
            .update({ pin_hash: pinHash, updated_at: db.fn.now() });
        return { temporaryPin: fixedPin };
    }
    async exit(customerId) {
        const db = (0, connection_1.getDb)();
        const customer = await db('customers').where({ id: customerId }).first();
        if (!customer) {
            throw new errorHandler_1.ApiError('Customer not found', 404);
    ...
    async list(sellerId, filters) {
        ...
        const customers = await Promise.all(rows.map(async (row) => {
            ...
            const customerProducts = await db('customer_products')
                ...
                .select('customer_products.product_id as productId', 'customer_products.quantity', 'customer_products.auto_entry as autoEntry', 'products.name as productName', 'products.unit');
            return {
                ...customer,
