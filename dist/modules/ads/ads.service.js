...
const targetingSchema = zod_1.z
    .object({
    role: zod_1.z.enum(['seller', 'admin', 'customer']).optional(),
})
    .passthrough()
    .optional();
const adSchema = zod_1.z.object({
    ...
    url: zod_1.z.string().url('Landing URL must be valid').optional(),
    expiresAt: zod_1.z.string().datetime().optional(), // Keep for backward compatibility
...
class AdsService {
    ...
    async listActive() {
        ...
        const allAds = await db('ads').select('*');
    ...
    async create(payload) {
        ...
        const row = await db('ads').where({ id }).first();
    ...
    async update(adId, payload) {
        ...
        await db('ads')
            ...
            targeting: typeof data.targeting !== 'undefined'
                ? data.targeting
    ...
    async getMonthlyStats(adId, year, month) {
        const db = (0, connection_1.getDb)();
        ...
        const stats = await query
            .select(db.raw('year, month, COUNT(*) as click_count'), db.raw('COUNT(DISTINCT user_id) as unique_users'))
            .groupBy('year', 'month')
            .orderBy('year', 'desc')
            .orderBy('month', 'desc');
        return stats.map((stat) => ({
            year: stat.year,
            month: stat.month,
            clickCount: parseInt(stat.click_count, 10),
            uniqueUsers: parseInt(stat.unique_users, 10),
        }));
    }
    async getAdStats(adId) {
        ...
        return {
            totalClicks: parseInt(totalClicks?.count || '0', 10),
