...
const updateSellerSchema = zod_1.z.object({
    ...
    businessName: zod_1.z.string().min(2, 'Business name is required').optional(),
    mobile: zod_1.z.string().regex(/^\d{10}$/, 'Mobile number must be 10 digits').optional(),
    status: zod_1.z.enum(['Active', 'Pending', 'Inactive']).optional(),
    subscriptionPlanId: zod_1.z
        .string()
        .trim()
        ...
        .optional()
        .nullable(),
    whatsappAvailable: zod_1.z.boolean().optional(),
...
const createSellerSchema = zod_1.z.object({
    ...
    status: zod_1.z.enum(['Active', 'Pending', 'Inactive']).default('Pending'),
    subscriptionPlanId: zod_1.z
        .string()
        .trim()
...
const listSellersSchema = zod_1.z.object({
    status: zod_1.z.enum(['Active', 'Pending', 'Inactive']).optional(),
...
class SellerService {
    ...
    async listSellers(filters) {
        ...
        const query = db('sellers as s')
            ...
            .select('s.id', 's.business_name', 's.status as seller_status', 's.subscription_plan_id', 'u.name as user_name', 'u.mobile as user_mobile', 'sp.name as plan_name');
        if (params.status) {
    ...
    async getDashboard(sellerId) {
        ...
        const seller = await db('sellers as s')
            .leftJoin('subscription_plans as sp', 's.subscription_plan_id', 'sp.id')
            .select('s.id', 's.business_name', 's.status', 'sp.name as plan_name', 'sp.price as plan_price', 'sp.billing_cycle')
            .where('s.id', sellerId)
            .first();
        if (!seller) {
        ...
        const pendingBillsRow = await db('bills')
            ...
            .select(db.raw('COUNT(id) as count'), db.raw('SUM(total_amount) as sum'))
            .first();
        ...
        const monthlyConsumptionRow = await db('daily_entries')
            ...
            .select(db.raw('SUM(quantity) as sum'), db.raw('COUNT(id) as count'))
            .first();
        ...
        const recentCollections = await db('seller_subscription_collections')
            ...
            .limit(5)
            .select('id', 'month', 'year', 'amount', 'status', 'due_date', 'paid_at', 'payment_mode', 'reference');
    ...
    async updateSeller(sellerId, payload) {
        ...
        if (data.mobile && data.mobile !== seller.user_mobile) {
            ...
            if (existingUser) {
                throw new errorHandler_1.ApiError('A user with this mobile number already exists', 409);
            }
        }
        ...
        try {
            ...
            if (!oldPlanId && planProvidedInPayload && !newPlanId) {
                ...
                await trx.rollback();
                throw new errorHandler_1.ApiError('Seller has no subscription plan. Please select a subscription plan when updating this seller.', 400);
            }
            ...
            if (oldPlanId !== newPlanId) {
                ...
                if (newPlanId) {
                    ...
                    if (!planExists) {
                        await trx.rollback();
                        throw new errorHandler_1.ApiError(`Subscription plan with ID ${newPlanId} not found`, 400);
                    }
            ...
            throw new errorHandler_1.ApiError(error instanceof Error ? `Unable to update seller: ${error.message}` : 'Unable to update seller', 500);
        }
    }
    ...
    async createSeller(payload) {
        ...
        const sellerId = (0, crypto_1.randomUUID)();
        ...
        try {
            ...
            if (!planExists) {
                await trx.rollback();
                throw new errorHandler_1.ApiError(`Subscription plan with ID ${subscriptionPlanId} not found`, 400);
            }
            ...
            throw new errorHandler_1.ApiError(error instanceof Error ? `Unable to create seller: ${error.message}` : 'Unable to create seller', 500);
        }
        const delivery = null;
        return {
            seller: {
    ...
    async updateSubscription(sellerId, payload) {
        const data = zod_1.z
            .object({
            planId: zod_1.z.string().min(1, 'Plan id required'),
            changedBy: zod_1.z.string().uuid().optional(),
        })
            .parse(payload);
        const db = (0, connection_1.getDb)();
