...
const collectionStatus = ['pending', 'overdue', 'paid'] as const;
type CollectionStatus = (typeof collectionStatus)[number];

const paymentModes = ['cash', 'online', 'bank', 'upi'] as const;
...
const listCollectionsSchema = z.object({
  status: z.enum(collectionStatus).optional(),
});

const formatStatus = (status: CollectionStatus): CollectionStatus => {
...
const getMonthlyAmount = (plan: { amount: number; billingCycle: 'monthly' | 'yearly' | string }) => {
...
class PaymentService {
  private mapCollectionRow(row: any) {
    return {
      id: row.id,
  ...
  private async fetchCollectionById(id: string) {
    const db = getDb();
  ...
  private async syncSubscriptionCollections() {
    const db = getDb();
    const now = new Date();
    ...
    await Promise.all(
      desiredEntries.map((entry) =>
        db('seller_subscription_collections')
          .where({
  ...
  async listSubscriptionCollections(filters?: unknown) {
  ...
  async markSubscriptionCollectionPaid(collectionId: string, payload: unknown) {
    ...
    if (collection.status === 'paid') {
      throw new ApiError('Collection already marked as paid', 400);
    }
