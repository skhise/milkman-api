...
import type { MessagingResult } from '../../services/messaging.service';
...
const sellerStatusMap: Record<string, 'pending' | 'active' | 'suspended'> = {
  Active: 'active',
  Pending: 'pending',
  Inactive: 'suspended',
};

const sellerDisplayStatusMap: Record<'pending' | 'active' | 'suspended', 'Pending' | 'Active' | 'Inactive'> = {
  pending: 'Pending',
  active: 'Active',
  suspended: 'Inactive',
};
...
class SellerService {
  ...
  private async createSellerSubscription(
    trx: ReturnType<typeof getDb>,
    sellerId: string,
    planId: string,
  ...
  async listSellers(filters?: unknown) {
    ...
    return rows.map((row) => {
      const rawStatus = row.seller_status as string | undefined;
      let statusKey: 'pending' | 'active' | 'suspended' = 'pending';
  ...
  async getDashboard(sellerId?: string) {
    ...
    const pendingBillsRow = await db('bills')
      ...
      .first<{ count: string | number; sum: string | number | null }>();
    ...
    const monthStartStr: string = monthStart.toISOString().split('T')[0]!;
    const monthEndStr: string = monthEnd.toISOString().split('T')[0]!;
    ...
    const monthlyConsumptionRow = await db('daily_entries')
      ...
      .first<{ sum: string | number | null; count: string | number }>();
  ...
  async updateProfile(sellerId: string, payload: unknown) {
  ...
  async updateSeller(sellerId: string, payload: unknown) {
    ...
    try {
      ...
      if (data.whatsappAvailable !== undefined) {
        const currentMetadata = seller.metadata ? JSON.parse(seller.metadata as string) : {};
      ...
      const rawStatus = updatedSeller.seller_status as string | undefined;
      let statusKey: 'pending' | 'active' | 'suspended' = 'pending';
      if (rawStatus === 'active' || rawStatus === 'suspended' || rawStatus === 'pending') {
        statusKey = rawStatus;
      }
  ...
  async listCustomers(sellerId: string, filters: unknown) {
    if (!sellerId) {
      throw new ApiError('Seller id is required', 400);
    }
  ...
  async createSeller(payload: unknown) {
    ...
    const delivery: MessagingResult | null = null;
  ...
  async updateSubscription(sellerId: string, payload: unknown) {
    const data = z
      .object({
