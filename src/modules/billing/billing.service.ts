...
class BillingService {
  ...
  async generate(sellerId: string, customerId: string, query: unknown) {
    const params = z.object({ month: z.string(), year: z.string() }).parse(query);
    const db = getDb();
    ...
    const customerProducts = await db('customer_products as cp')
      ...
      .select(
        ...
        'cp.product_id',
        'cp.quantity as assigned_quantity',
    ...
    const deliveredQuantitiesByProduct = new Map<string, number>();
    const deliveredDaysByProduct = new Map<string, number>(); // Track actual delivery days
    ...
    if (customerProducts.length === 0 && dailyEntries.length > 0) {
      ...
      const entriesByProduct = new Map<string, { quantity: number; dates: string[] }>();
      for (const entry of dailyEntries) {
        const productId = entry.product_id;
        const quantity = Number(entry.quantity || 0);
    ...
    const previousBills = await db('bills')
      ...
      .sum<{ sum: string | null }>('total_amount as sum')
      .first();
  ...
  async update(billId: string, payload: unknown) {
    const data = z.object({ notes: z.string().optional(), adjustments: z.array(z.any()).optional() }).parse(payload);
    const db = getDb();
  ...
  async sendReminder(billId: string, payload: unknown) {
    ...
    if (bill.user_id) {
      try {
        ...
        await notificationService.sendToUser(
          ...
          `Your bill for ${monthName} ${bill.year} (₹${Number(bill.total_amount).toFixed(2)}) is pending. Please make the payment.`,
          {
            type: 'bill_reminder',
            billId: billId,
  ...
  async markPaid(billId: string, payload: unknown) {
    ...
    if (bill.status === 'paid') {
      throw new ApiError('Bill already marked as paid', 400);
    }
  ...
  async markUnpaid(billId: string) {
    const db = getDb();
    const bill = await db('bills').where({ id: billId }).first();
    if (!bill) {
      throw new ApiError('Bill not found', 404);
    ...
    if (bill.status !== 'paid') {
      throw new ApiError('Bill is not marked as paid', 400);
    }
  ...
  async listBills(sellerId: string, filters?: { year?: string; month?: string; status?: string; customerId?: string }) {
    const db = getDb();
  ...
  async generateMonthlyBillsForSeller(sellerId: string, month: string, year: string) {
    const db = getDb();
   ...
   */
  async generateMonthlyBillsForAllSellers(month: string, year: string) {
    const db = getDb();
  ...
  async getBillWithItems(billId: string) {
    const db = getDb();
  ...
  private async getBillDetails(billId: string) {
    const db = getDb();
    ...
    if (items.length === 0) {
      ...
      items.forEach((item) => {
        ...
        doc
          ...
          .text(
            `  Qty: ${Number(item.quantity).toFixed(2)} @ ₹${Number(item.unit_price).toFixed(2)}  Total: ₹${Number(item.total_price).toFixed(2)}`,
          );
      });
    }
